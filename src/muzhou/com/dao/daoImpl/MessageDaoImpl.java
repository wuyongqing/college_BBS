...
public class MessageDaoImpl extends DAO<MessageBean> implements MessageDao {
    public List<Map<String, Object>> getMessageList(int userid) {
        String sql = "select m.mid,m.time,m.send_userid,m.qid,q.title,m.type,u.username from message m,user u,question q where (q.qid=m.qid or m.qid is null) and u.userid=send_userid and m.rece_userid = ? group by m.mid order by m.time desc";


        return getForMapList(sql, userid);
    }
